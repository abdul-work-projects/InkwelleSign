...
import { db, newId, nowIso, DEMO_MODE, DEMO_SECRET } from './db.js';
...
import { issueStatelessSession, readStatelessSession } from './session-token.js';
...
export function createSession(userId, ip, userAgent) {
  if (DEMO_MODE) {
    ...
    return { raw: issueStatelessSession(DEMO_SECRET, userId, expires.getTime()), expires };
  }
...
export function resolveSession(rawToken) {
  ...
  if (DEMO_MODE) {
    const userId = readStatelessSession(DEMO_SECRET, rawToken);
    ...
    if (!u) return null;
    return {
      sessionId: 'demo', id: u.id, orgId: u.org_id, email: u.email, name: u.name,
      role: u.role, orgName: u.org_name, orgSlug: u.org_slug,
    };
  }
