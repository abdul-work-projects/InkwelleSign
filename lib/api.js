...
import { ensureDemoSeeded } from './demo-seed.js';
...
export function withAuth(handler, { minRole = 'viewer' } = {}) {
  return async (request, context) => {
    try {
      ...
      if (err?.code === 'STORAGE_UNAVAILABLE') return fail(err.message, 503);
...
/**
 * Uniform error handling for handlers that do their own authentication (sign-in,
 * registration, scheduled jobs). Without this a storage failure surfaces as a bare 500
...
export function withRoute(handler) {
  return async (request, context) => {
    try {
      ...
      if (err?.code === 'STORAGE_UNAVAILABLE') return fail(err.message, 503);
      console.error('[api]', err);
      return fail(err?.message || 'Unexpected server error', err?.status || 500);
    }
  };
}
