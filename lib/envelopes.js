...
import { sha256, randomToken, hmacSha256 } from './crypto.js';
...
function issueToken(recipientId) {
  // In demo mode the token is derived from the recipient, so the same link resolves in
  // every function that seeded its own copy of the workspace.
  const raw = DEMO_MODE ? hmacSha256(DEMO_SECRET, `token:${recipientId}`) : randomToken(32);
